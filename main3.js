...
function calculateTotal() {
    ...
    var depLevel = '';
    if (totalMarks >= 0 && totalMarks <= 9) {
        depLevel = 'Normal';
    } else if (totalMarks >= 10 && totalMarks <= 13) {
        depLevel = 'Mild Depression';
    } else if (totalMarks >= 14 && totalMarks <= 20) {
        depLevel = 'Moderate Depression';
    }
    else if (totalMarks >= 21 && totalMarks <= 27) {
        depLevel = 'Severe Depression';
    } 
    else {
        depLevel = 'Extremely Severe Stress';
    }

    // Display the total marks and stress level
    var totalMarksElement = document.getElementById("totalMarks");
    totalMarksElement.innerText = "Total Marks: " + totalMarks + ", Depression Level: " + depLevel;
    totalMarksElement.style.display = "block";
